# 《RISC-V操作系统进展》·第007期·2023年05月01日

## 卷首语

四月，openEuler Developer Day 2023在上海顺利召开。openEuler 工业领域操作系统筹备委员会成立，中国科学院软件研究所成为 openEuler 工业领域操作系统筹备委员会首批成员单位。

软件所吴伟老师在多样性计算分论坛发布了《openEuler RISC-V 2023-2025:领先并超越》的主题演讲；纪涛、陆旭凡两位老师在开发者服务分论坛中分别发表了《OEPKGS 社区开放软件包服务》、《LLVM IR 简介及近期演变介绍》为主题的报告。

ROS、RISC-V SIG 开放工作会议联合举办，商讨了后续在RISC-V架构上适配ROS1和ROS2的工作计划，同时RISC-V SIG还计划在23.09版本将RISC-V架构合入主线，成为默认支持的主流架构之一。此外，LLVM平行宇宙项目也计划在23.09发布一个测试镜像。

五月，愿各位继续实现自身技术和创意上的突破。



## 本期亮点

- [openEuler RISC-V 23.03 创新版本亮相：全面提升硬件兼容性和桌面体验。](https://mp.weixin.qq.com/s/VwTbob0wr6A5A_8nEp2H4w)

- openEuler RISC-V 团队一起亮相Yutobe openEuler。

  [openEuler Techday EP04 — Leap into the future with the openEuler RISC-V SIG](https://www.youtube.com/watch?v=ehAik7Svmtk)

- [openEuler 成功适配 SG2042 服务器板卡，加速软件包构建。](https://mp.weixin.qq.com/s/Te1RPG_DlMYP3InmJrU8sw)

- [openEuler RISC-V 已适配 LicheePi 4A ，欢迎下载测试版镜像试用。](https://forum.openeuler.org/t/topic/1288?u=yayaleyyl) 

- 软件所协办 openEuler Developer Day 2023，分享创新成果，协同规划发展。

  

## Infra

### wangliu
1.新增文档
- [openEuler helloworld打包文档](https://gitee.com/ouuleilei/working-documents/blob/2f87907a22d79c0c774b7f214f36c7cf3c0e0a17/%E6%96%87%E6%A1%A3/openEuler%E6%89%93%E5%8C%85/openEuler%20helloworld%E6%89%93%E5%8C%85%E6%96%87%E6%A1%A3.md)

2.新增脚本
- [监控设备存活并带有tg通知功能的脚本](https://gitee.com/ouuleilei/unmatchedmonitor)

## openEuler

### lvxiaoqian
- openqa+unmatched实现命令行测试和通过vnc的GUI测试
  - 部署文档
    - https://gitee.com/lvxiaoqian/memo/blob/master/openQA+unmatched.md
  - 代码修改
    - https://gitee.com/lvxiaoqian/os-autoinst-needles-openeuler/commit/5184de4324e32f9421274419488dc4217a892792
    - https://gitee.com/lvxiaoqian/os-autoinst-distri-openeuler/commit/a0f428ec54327429de5e13c932ce1ba72cdb5e7f

### mingzheng

- PR

  - [BCC rewriter: Init only the requested parameters](https://github.com/iovisor/bcc/pull/4601)

  - [BCC libbpf-tools: Bump riscv vmlinux.h to v6.2](https://github.com/iovisor/bcc/pull/4600)

  - [thead-kernel RISC-V: Enable container related kernel configs](https://github.com/revyos/thead-kernel/pull/1)

### misaka00251

这个月主要做了以下事情：

 - ODD 2023 & 开源大赛准备
 - 将去年添加的 texlive 家族依赖加入 mainline & Epol，为 `texlive-scheme-basic`, `texlive-scheme-medium` 和 `texlive-scheme-full` 能够正常被安装做基础。
 - 完善 2303 发版最后的工作，并为 2303 的维护做好一系列准备

#### 已做

<details>
  <summary>点击展开</summary>

 - https://gitee.com/src-openeuler/eggo/pulls/57
 - https://gitee.com/openeuler/RISC-V/pulls/635
 - https://gitee.com/openeuler-risc-v/golang/pulls/3
 - https://gitee.com/openeuler/release-management/pulls/1021
 - https://gitee.com/openeuler/release-management/pulls/1028
 - https://gitee.com/src-openeuler/perl-PadWalker/pulls/2
 - https://gitee.com/src-openeuler/perl-Variable-Magic/pulls/3
 - https://gitee.com/src-openeuler/perl-Ref-Util-XS/pulls/4

</details>

#### 其它

 - [2303 源码矫正记录](https://docs.qq.com/sheet/DZENFY3B2eXl1UFhN?tab=BB08J2)

### jchzhou

### jchzhou

四月主要活动内容如下：

-  [23.03 版本](
https://mirror.iscas.ac.cn/openeuler-sig-riscv/openEuler-RISC-V/preview/openEuler-23.03-V1-riscv64/) 发版以及对应的 CI 修复和改进
- 回合上游对应 CI 改进
- 硬件相关软件包 (kernel, opensbi, uboot) 修复、调整和改进
- 一些软件包的升级/提交上游/打包
- 相关会议、比赛的准备和出勤
- 平行宇宙计划初上手

rust 目前状态

- 1.65.0 ✔️ 1.66.1 ✔️ 1.67.1 ✔️ 1.69.0 (WIP)

#### 已提交 PR

<details>
  <summary>点击展开</summary>

- merged: musl 配置相关的小修改 `configure.py: add flag for riscv{64,32}gc musl-root`
  - https://github.com/rust-lang/rust/pull/110779

- open: 尝试将 rust 包的修改推到 openEuler 主线，顺便升级 1.69.0
  - https://gitee.com/src-openeuler/rust/pulls/85

</details>

#### 尝试中/进行中/暂未提交

<details>
  <summary>点击展开</summary>

- 软件包

  - clang-12 (parallel)
  - rust-bindgen (blocked)
  - edk2 (packaging)
- 其他
  - 申请成为 rust sig 的提交者/维护者

</details>

### Jingwiw


四月主要活动内容如下：

- 参加 oDD 大会，并且分享了关于 oerv 创新反向的工作报告
- firefox 维护升级至 111 版本，成功构建 
- 代码推到112.02版本，该版本有重要bug修复，并且将 riscv jit 相关更新 patch 化 合入构建
- mesa 维护升级至 23.0.3 版本
- dav1d 项目初始化，提交关于 初始化 checkasm 的代码

#### 已提交 PR

<details>
  <summary>点击展开</summary>

- open: dav1d 上游初始化
  - https://code.videolan.org/videolan/dav1d/-/merge_requests/1505

</details>

#### 尝试中/进行中/暂未提交

<details>
  <summary>点击展开</summary>

- 软件包

  - firefox111 (succeed)
    - https://build.tarsier-infra.com/package/show/home:Jingwiw:Build-Firefox/firefox111
  - mesa23.0.3 (succeed)
    - https://build.tarsier-infra.com/package/show/home:Jingwiw:MultiMedia/Mesa-23
  - firefox112 (building)
    - https://build.tarsier-infra.com/package/show/home:Jingwiw:Build-Firefox/firefox112

- 其他
  - 和 @eastdong 老师一起加速 qt6 组件的工作
</details>


### EastDong
四月的工作进展：
  - 学习 openEuler 这边的工作流程
  - 在 @jingwiw 老师的指导下完善 Qt6 相关软件包，目前已经完善 30 个软件包
  - 提交软件包 libb2 到 openEuler，目前已经合并
  - 将 Qt6 的软件包升级为 6.5.0 并提交给 openEuler

#### 已提交 PR
- libb2 (qt6-qtbase 的依赖): https://gitee.com/src-openeuler/libb2/pulls/1 
- qt6-qtbase: https://gitee.com/src-openeuler/qt6-qtbase/pulls/1
#### 尝试中/进行中/暂未提交
- 已完成，暂未提交上游(等待 qtbase 被合并):
  - qt6-qt3d: https://gitee.com/East_x/qt6-qt3d
  - qt6-speech: https://gitee.com/East_x/qt6-speech
  - qt6-multimedia: https://gitee.com/East_x/qt6-multimedia
  - qt6-qtshadertools: https://gitee.com/East_x/qt6-qtshadertools
  - qt6-qtimageformats: https://gitee.com/East_x/qt6-qtimageformats
- 进行中:
  - qt6-qttools: https://gitee.com/East_x/qt6-qttools


### laokz

<details>
  <summary>继续整理提交给openEuler的PR。点击展开</summary>

- https://gitee.com/src-openeuler/libvpx/pulls/6
  
- https://gitee.com/src-openeuler/rocksdb/pulls/20
  
- https://gitee.com/openeuler/openEuler-rpm-config/pulls/15  merged
  
- https://gitee.com/src-openeuler/kmod-kvdo/pulls/25
  

</details>

<details>
  <summary>分析提出了解决“samba找不到动态库问题”设想。点击展开</summary>

- https://gitee.com/laokz/oerv/blob/master/glibc-libc6.md
  
- https://gitee.com/src-openeuler/glibc/issues/I6Y77W?from=project-issue
  

</details>

### Yurish
四月进展：
  - 学习打包流程
  - dart文档内容复现
  - 技术分享 ：https://gitee.com/yurish/plct-work/blob/master/2023-04/%E5%85%B3%E4%BA%8E%E6%B5%8B%E8%AF%95%E6%B5%81%E7%A8%8B%E7%9A%84%E6%8E%A2%E8%AE%A8%E4%B8%8E%E4%BC%98%E5%8C%96%EF%BC%88%E5%88%86%E4%BA%AB%EF%BC%89.pptx
  - 测试组内openQA工作协调
  - ODD介绍内容 ：https://gitee.com/yurish/plct-work/blob/master/2023-04/%E8%87%AA%E6%88%91%E4%BB%8B%E7%BB%8D.pptx
  - 社区内容学习


### Interns

#### PR

- openQA xfce 测试 [测试](https://gitee.com/lvxiaoqian/os-autoinst-distri-openeuler/pulls/4) [Needles](https://gitee.com/lvxiaoqian/os-autoinst-needles-openeuler/pulls/3) [文档更新](https://github.com/ArielHeleneto/RISCV-testcase/pull/59)

- 增加密钥长度选项以解决 libkcapi 在使用 SM4 算法进行加密时由于密钥长度错误造成 的 EINVAL [Issue #150](https://github.com/smuellerDD/libkcapi/issues/150) [Pull Request](https://github.com/smuellerDD/libkcapi/pull/151)
- openEuler libkcapi 包应用补丁 [libkcapi:添加 Patch Add-option-key-len](https://gitee.com/src-openeuler/libkcapi/pulls/49)  @microseyuyu   open

#### 验证与测试

- [Fix openeuler riscv 2203LTS/2303V1 version on visionfive2](https://gitee.com/samuel_yuan/riscv-openeuler-visionfive/blob/master/chapter1-Build-openeuler-on-visionfive/Fix-openeuler-riscv-2203LTS-classical-version-on-visionfive2.md)  @samuel_yuan
- [Verify wine-ce for openeuler on visionfive2](https://gitee.com/samuel_yuan/riscv-openeuler-visionfive/blob/master/chapter2-Verify-openeuler-GUI-APP-on-visionfive/Verify-wine-ce-for-openeuler-on-visionfive.md) @samuel_yuan



## openEuler-LLVM平行宇宙

### huangxiaoya

1. [zerofree：gcc / clang 错误](https://gitee.com/openeuler-risc-v/zerofree/pulls/3)
2. [A-Tune-BPF-Collection：文件名引用错误](https://gitee.com/openeuler-risc-v/A-Tune-BPF-Collection/pulls/2)
3. [byaccj：gcc / clang 错误](https://gitee.com/openeuler-risc-v/byaccj/pulls/3)
4. [container-exception-logger：gcc / clang 错误](https://gitee.com/openeuler-risc-v/container-exception-logger/pulls/3)
5. [dietlibc：gcc / clang 错误](https://gitee.com/openeuler-risc-v/dietlibc/pulls/2)
6. [ed：gcc / clang 错误](https://gitee.com/openeuler-risc-v/ed/pulls/2)
7. [efivar：gcc / clang 错误](https://gitee.com/openeuler-risc-v/efivar/pulls/4)
8. [fxload：gcc / clang 错误](https://gitee.com/openeuler-risc-v/fxload/pulls/2)
9. [iputils：gcc / clang 错误](https://gitee.com/openeuler-risc-v/iputils/pulls/2)
10. [kpatch：gcc / clang 及 CFLAGS 错误](https://gitee.com/openeuler-risc-v/kpatch/pulls/1)
11. [mac-robber：gcc / clang 错误](https://gitee.com/openeuler-risc-v/mac-robber/pulls/2)
12. [net-tools：gcc / clang 错误](https://gitee.com/openeuler-risc-v/net-tools/pulls/3)
13. [pngcrush：gcc / clang 错误](https://gitee.com/openeuler-risc-v/pngcrush/pulls/1)
14. [pptp：gcc / clang 错误](https://gitee.com/openeuler-risc-v/pptp/pulls/1)
15. [vsftpd：gcc / clang 错误](https://gitee.com/openeuler-risc-v/vsftpd/pulls/1)
16. [wireless-tools：gcc / clang 错误](https://gitee.com/openeuler-risc-v/wireless-tools/pulls/1)
17. [zip：gcc / clang 错误](https://gitee.com/openeuler-risc-v/zip/pulls/1)
18. [tinycdb：gcc / clang 错误](https://gitee.com/openeuler-risc-v/tinycdb/pulls/1)
19. [tss2：gcc / clang 错误](https://gitee.com/openeuler-risc-v/tss2/pulls/4)
20. [libx86emu：gcc / clang 错误](https://gitee.com/openeuler-risc-v/libx86emu/pulls/1)
21. [vconfig：gcc / clang 错误](https://gitee.com/openeuler-risc-v/vconfig/pulls/2)
22. [symlinks：gcc / clang 错误](https://gitee.com/openeuler-risc-v/symlinks/pulls/1)
23. [libkeepalive：gcc / clang 错误](https://gitee.com/openeuler-risc-v/libkeepalive/pulls/1)
24. [lua-lpeg：gcc / clang 错误](https://gitee.com/openeuler-risc-v/lua-lpeg/pulls/1)
25. [traceroute：gcc / clang 错误](https://gitee.com/openeuler-risc-v/traceroute/pulls/2)
26. [libserf：gcc / clang 错误](https://gitee.com/openeuler-risc-v/libserf/pulls/2)
27. [sgpio：gcc / clang 错误](https://gitee.com/openeuler-risc-v/sgpio/pulls/1)
28. [lm_sensors：gcc / clang 错误](https://gitee.com/openeuler-risc-v/lm_sensors/pulls/2)
29. [pigz：gcc / clang 错误](https://gitee.com/openeuler-risc-v/pigz/pulls/3)
30. [java-service-wrapper：gcc / clang 错误](https://gitee.com/openeuler-risc-v/java-service-wrapper/pulls/5)
31. [linuxptp：gcc / clang 错误](https://gitee.com/openeuler-risc-v/linuxptp/pulls/3)
32. [python-pycurl：gcc / clang 错误](https://gitee.com/openeuler-risc-v/python-pycurl/pulls/3)
33. [jna：gcc / clang 错误](https://gitee.com/openeuler-risc-v/jna/pulls/1)
34. [openresty：gcc / clang 错误](https://gitee.com/openeuler-risc-v/openresty/pulls/2)
35. [strace：gcc / clang 错误](https://gitee.com/openeuler-risc-v/strace/pulls/1)
36. [environment-modules：gcc / clang 错误](https://gitee.com/openeuler-risc-v/environment-modules/pulls/1)
37. [ftp：gcc / clang 错误](https://gitee.com/openeuler-risc-v/ftp/pulls/1)
38. [jimtcl：gcc / clang 错误]()
39. [Judy：gcc / clang 错误](https://gitee.com/openeuler-risc-v/Judy/pulls/1)
40. [lzip：gcc / clang 错误](https://gitee.com/openeuler-risc-v/lzip/pulls/1)
41. [OpenCSD：gcc / clang 错误](https://gitee.com/openeuler-risc-v/OpenCSD/pulls/1)
42. [qt-assistant-adp：gcc / clang 错误](https://gitee.com/openeuler-risc-v/qt-assistant-adp/pulls/1)
43. [squid：spec 文件未声明构建所必要依赖](https://gitee.com/openeuler-risc-v/squid/pulls/3)
44. [tinyxml：gcc / clang 错误](https://gitee.com/openeuler-risc-v/tinyxml/pulls/1)
45. [ttmkfdir：gcc / clang 错误](https://gitee.com/openeuler-risc-v/ttmkfdir/pulls/1)
46. [fcoe-utils：gcc / clang 错误](https://gitee.com/openeuler-risc-v/fcoe-utils/pulls/1)
47. [ftgl：gcc / clang 错误](https://gitee.com/openeuler-risc-v/ftgl/pulls/1)
48. [glib：gcc / clang 错误](https://gitee.com/openeuler-risc-v/glib/pulls/1)
49. [gnutls：gcc / clang 错误](https://gitee.com/openeuler-risc-v/gnutls/pulls/1)
50. [gtk：gcc / clang 错误](https://gitee.com/openeuler-risc-v/gtk/pulls/1)
51. [gutenprint：gcc / clang 错误](https://gitee.com/openeuler-risc-v/gutenprint/pulls/1)
52. [hplip：gcc / clang 错误](https://gitee.com/openeuler-risc-v/hplip/pulls/1)
53. [kim-api：gcc / clang 错误](https://gitee.com/openeuler-risc-v/kim-api/pulls/1)
54. [kunpengsecl：文件权限错误](https://gitee.com/openeuler-risc-v/kunpengsecl/pulls/1)
55. [leveldb：gcc / clang 错误](https://gitee.com/openeuler-risc-v/leveldb/pulls/2)
56. [lensfun：gcc / clang 错误](https://gitee.com/openeuler-risc-v/lensfun/pulls/1)
57. [libgdiplus：C 语言语法错误](https://gitee.com/openeuler-risc-v/libgdiplus/pulls/1)
58. [libisoburn：gcc / clang 错误](https://gitee.com/openeuler-risc-v/libisoburn/pulls/1)
59. [liblockfile：Makefile 不支持指定 --prefix 选项](https://gitee.com/openeuler-risc-v/liblockfile/pulls/1)
60. [libpfm：gcc / clang 错误](https://gitee.com/openeuler-risc-v/libpfm/pulls/1)
61. [libreswan：gcc / clang 及 CFLAGS、USERLINK 错误](https://gitee.com/openeuler-risc-v/libreswan/pulls/1)
62. [libuser：C 语言语法错误](https://gitee.com/openeuler-risc-v/libuser/pulls/1)
63. [libtpms：gcc / clang 及 CFLAGS 错误](https://gitee.com/openeuler-risc-v/libtpms/pulls/1)

### sunyuechi

1. [lua-filesystem: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/lua-filesystem/pulls/1)
1. [ladspa: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/ladspa/pulls/2)
1. [procinfo: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/procinfo/pulls/1)
1. [epstool: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/epstool/pulls/1)
1. [haproxy: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/haproxy/pulls/3)
1. [native-platform: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/native-platform/pulls/1)
1. [qgnomeplatform: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/qgnomeplatform/pulls/1)
1. [snappy-java: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/snappy-java/pulls/1)
1. [webbench: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/webbench/pulls/1)
1. [nvme-snsd: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/nvme-snsd/pulls/1)
1. [nethogs: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/nethogs/pulls/1)
1. [attest-tools: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/attest-tools/pulls/2)
1. [esc: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/esc/pulls/1)
1. [system-storage-manager: 修复 python逻辑错误](https://gitee.com/openeuler-risc-v/system-storage-manager/pulls/1)
1. [cdparanoia: 修复 类型不存在错误](https://gitee.com/openeuler-risc-v/cdparanoia/pulls/1)
1. [filebench: 修复 undefined ref错误](https://gitee.com/openeuler-risc-v/filebench/pulls/1)
1. [perl-Devel-Size: 修复 strip permission错误](https://gitee.com/openeuler-risc-v/perl-Devel-Size/pulls/1)
1. [perl-HTML-Parser: 修复 strip permission错误](https://gitee.com/openeuler-risc-v/perl-HTML-Parser/pulls/1)
1. [perl-TermReadKey: 修复 strip permission错误](https://gitee.com/openeuler-risc-v/perl-TermReadKey/pulls/1)
1. [libimagequant: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/libimagequant/pulls/1)
1. [pesign: 修复 clang 构建错误](https://gitee.com/openeuler-risc-v/pesign/pulls/1)

### Interns

#### linjingdong

1. [ostree:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/ostree/pulls/2)  @jammyjellyfish
2. [valgrind:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/valgrind/pulls/8)  @jammyjellyfish
3. [busybox:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/busybox/pulls/2)  @jammyjellyfish
4. [lmdb:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/lmdb/pulls/1)  @jammyjellyfish
5. [libcap:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/libcap/pulls/1)  @jammyjellyfish
6. [ladspa:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/ladspa/pulls/4)  @jammyjellyfish
7. [kiwi:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/kiwi/pulls/1)  @jammyjellyfish
8. [ipvsadm:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/ipvsadm/pulls/1)  @jammyjellyfish
9. [initscripts:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/initscripts/pulls/1)  @jammyjellyfish
10. [grubby:支持 clang 编译](https://gitee.com/openeuler-risc-v/grubby/pulls/1)  @jammyjellyfish
11. [genwqe-tools:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/genwqe-tools/pulls/2)  @jammyjellyfish
12. [glew:支持 clang 编译](https://gitee.com/openeuler-risc-v/glew/pulls/2)  @jammyjellyfish
13. [dcraw:修改 spec 支持指定 CC 编译](https://gitee.com/openeuler-risc-v/dcraw/pulls/1)  @jammyjellyfish
14. [dbxtool:支持指定 CC 编译](https://gitee.com/openeuler-risc-v/dbxtool/pulls/1)  @jammyjellyfish
15. [bwa:修改 spec 支持指定 CC 编译](https://gitee.com/openeuler-risc-v/bwa/pulls/2)  @jammyjellyfish
16. [audit:修改 spec 支持指定 CC 编译](https://gitee.com/openeuler-risc-v/audit/pulls/1)  @jammyjellyfish
17. [lzo:修改 spec 支持指定 CC 编译-mega](https://gitee.com/openeuler-risc-v/lzo/pulls/3)  @jammyjellyfish   merged
18. [brasero:修复 clang 构建错误](https://gitee.com/src-openeuler/brasero/pulls/6)  @jammyjellyfish
19. [galera:修复 clang 构建错误](https://gitee.com/src-openeuler/galera/pulls/31)  @jammyjellyfish
20. [dmraid:修复 clang 构建错误](https://gitee.com/src-openeuler/dmraid/pulls/8)  @jammyjellyfish
21. [ltrace:修复 clang 构建错误](https://gitee.com/src-openeuler/ltrace/pulls/16)  @jammyjellyfish
22. [valgrind:支持指定 CC 编译](https://gitee.com/src-openeuler/valgrind/pulls/26)  @jammyjellyfish
23. [cjose:修复 clang 构建错误](https://gitee.com/src-openeuler/cjose/pulls/4)  @jammyjellyfish
24. [clucene:修复 clang 构建错误](https://gitee.com/src-openeuler/clucene/pulls/3)  @jammyjellyfish
25. [certmonger:修复 clang 构建错误](https://gitee.com/src-openeuler/certmonger/pulls/13)  @jammyjellyfish
26. [boost:支持指定 CC 构建](https://gitee.com/src-openeuler/boost/pulls/115)  @jammyjellyfish
27. [dtc:支持 clang 构建，移除 CFLAGS 中的 linker 选项](https://gitee.com/src-openeuler/dtc/pulls/17)  @jammyjellyfish
28. [dpdk:支持 clang 构建，移除 CFLAGS 中的 linker 选项](https://gitee.com/src-openeuler/dpdk/pulls/356)  @jammyjellyfish
29. [dvdplusrw-tools:支持指定 CC 编译，修复 clang 构建错误](https://gitee.com/src-openeuler/dvdplusrw-tools/pulls/4)  @jammyjellyfish
30. [chromaprint:修改 spec 文件步骤，支持指定 CC 编译](https://gitee.com/src-openeuler/chromaprint/pulls/4)  @jammyjellyfish
31. [lmdb:支持指定 CC 编译](https://gitee.com/src-openeuler/lmdb/pulls/34)  @jammyjellyfish
32. [lsof:支持指定 CC 编译](https://gitee.com/src-openeuler/lsof/pulls/40)  @jammyjellyfish
33. [lua-filesystem:支持指定 CC 编译](https://gitee.com/src-openeuler/lua-filesystem/pulls/6)  @jammyjellyfish
34. [pciutils:支持指定 CC 编译](https://gitee.com/src-openeuler/pciutils/pulls/31)  @jammyjellyfish
35. [procinfo:支持指定 CC 编译](https://gitee.com/src-openeuler/procinfo/pulls/8)  @jammyjellyfish
36. [sendmail:支持指定 CC 编译](https://gitee.com/src-openeuler/sendmail/pulls/57)  @jammyjellyfish
37. [sip:支持 clang 编译](https://gitee.com/src-openeuler/sip/pulls/12)  @jammyjellyfish
38. [ustr:支持指定 CC 编译](https://gitee.com/src-openeuler/ustr/pulls/6)  @jammyjellyfish
39. [wpa_supplicant:支持指定 CC 编译](https://gitee.com/src-openeuler/wpa_supplicant/pulls/38)  @jammyjellyfish
40. [libcap:支持指定 CC 编译](https://gitee.com/src-openeuler/libcap/pulls/39)  @jammyjellyfish
41. [ladspa:支持指定 CC 编译](https://gitee.com/src-openeuler/ladspa/pulls/20)  @jammyjellyfish
42. [ipvsadm:支持指定 CC 编译](https://gitee.com/src-openeuler/ipvsadm/pulls/13)  @jammyjellyfish
43. [grubby:支持 clang 编译](https://gitee.com/src-openeuler/grubby/pulls/46)  @jammyjellyfish
44. [glew:支持 clang 编译](https://gitee.com/src-openeuler/glew/pulls/20)  @jammyjellyfish
45. [genwqe-tools:支持指定 CC 编译](https://gitee.com/src-openeuler/genwqe-tools/pulls/2)  @jammyjellyfish
46. [crash-trace-command:支持指定 CC 编译](https://gitee.com/src-openeuler/crash-trace-command/pulls/11)  @jammyjellyfish
47. [crash-gcore-command:支持指定 CC 编译](https://gitee.com/src-openeuler/crash-gcore-command/pulls/7)  @jammyjellyfish
48. [b43-tools:支持指定 CC 编译](https://gitee.com/src-openeuler/b43-tools/pulls/7)  @jammyjellyfish
49. [kiwi:支持指定 CC 编译](https://gitee.com/src-openeuler/kiwi/pulls/106)  @jammyjellyfish
50. [busybox:修改 spec 支持指定 CC 编译](https://gitee.com/src-openeuler/busybox/pulls/83)  @jammyjellyfish
51. [audit:修改 spec 支持指定 CC 编译](https://gitee.com/src-openeuler/audit/pulls/78)  @jammyjellyfish
52. [bwa:支持指定 CC 编译](https://gitee.com/src-openeuler/bwa/pulls/4)  @jammyjellyfish
53. [byaccj:支持指定 CC 编译](https://gitee.com/src-openeuler/byaccj/pulls/6)  @jammyjellyfish
54. [dbxtool:支持 clang 构建](https://gitee.com/src-openeuler/dbxtool/pulls/9)  @jammyjellyfish
55. [dcraw:修改 spec 支持指定 CC 编译](https://gitee.com/src-openeuler/dcraw/pulls/7)  @jammyjellyfish
56. [elfutils:修复 clang 构建错误](https://gitee.com/src-openeuler/elfutils/pulls/97)  @jammyjellyfish    merged
57. [ypserv:修复 clang 构建错误](https://gitee.com/src-openeuler/ypserv/pulls/9)  @jammyjellyfish    merged
58. [tree:支持指定 CC 编译](https://gitee.com/src-openeuler/tree/pulls/28)  @jammyjellyfish    merged
59. [jbigkit:支持指定 CC 编译](https://gitee.com/src-openeuler/jbigkit/pulls/20)  @jammyjellyfish    merged
60. [initscripts:支持指定 CC 编译](https://gitee.com/src-openeuler/initscripts/pulls/40)  @jammyjellyfish    merged
61. [gsm:支持指定 CC 编译](https://gitee.com/src-openeuler/gsm/pulls/9)  @jammyjellyfish    merged



## openEuler-ROS

### zhengjunjie
#### 已提交 PR
- 修复 guile 可执行文件命名 https://gitee.com/src-openeuler/guile/pulls/22
- 增加 python3-lttng-ust (ros2 依赖) https://gitee.com/src-openeuler/lttng-ust/pulls/17
- urdfdom_headers https://gitee.com/src-openeuler/urdfdom_headers/pulls/3
- 给 rospkg 增加 openeuler 支持 https://github.com/ros-infrastructure/rospkg/pull/260
- 给 rosdep 增加 openeuler 支持 https://github.com/ros-infrastructure/rosdep/pull/919
- 给 rosdistro 增加 openeuler 包 https://github.com/ros/rosdistro/pull/36933
#### issues
- openeuler 的 os-release 文件不规范 https://gitee.com/openeuler/community/issues/I6Y25M
- openeuler 仓库链接问题 https://gitee.com/openeuler/community/issues/I6XZ8I
#### 尝试中/进行中/暂未提交
- 尚未提交
  - libspnav: https://gitee.com/Z572/libspnav-spec
  - uncrustify: https://gitee.com/Z572/uncrustify-spec
  - spdlog: https://build.openeuler.openatom.cn/package/show/home:Z572/spdlog
#### 学习笔记
- ros，ros2 https://github.com/Z572/Tarsier-Work/blob/main/ros%E5%AD%A6%E4%B9%A0.org
### wangbotao
4月份主要做了以下几方面的事情:
- 完成测试小队的交接
- 进入ros小队
- 学习打包和osc相关的内容
- 出差到odd
- 学习ros2相关的知识
#### 在测试小队的相关产出
##### pr相关
- [59 更新之前自己负责的一部分软件测试](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/59)

- [58 同步自己测试过的内容](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/58)

- [52 更新了表格里自己负责的一些测试信息](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/52)

- [50 更新vision v的一些测试用例](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/50)

- [41 增加了visionV 烧录镜像的ubuntu说明](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/41)

- [37 修改vision v说明](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/37)

##### issue相关
- [23.03kubekey部署 KubeSphere 集群失败](https://gitee.com/openeuler/RISC-V/issues/I6TVV6)

- [I6TFXV 23.03 vision five安装ukui桌面，进入桌面，两个画面循环显示](https://gitee.com/openeuler/RISC-V/issues/I6TFXV)

- [I6V7YG 23.03缺少软件包mindspore](https://gitee.com/openeuler/RISC-V/issues/I6V7YG)

#### ros相关工作
- [ros学习笔记](https://github.com/q1248/some_tasks/blob/master/ros%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0/ROS.md)
- 打包这月只是在分析一些依赖，还未真正开始，下个月进行此项工作
### liyongtai

4 月份主要做了以下事情:
- oERV 2303 发版前的测试
- 转入 ROS 小队前的测试工作交接
- 学习 ROS 相关知识
- 出差到 ODD
- 学习打包和 obs 使用

在测试小队的相关工作:

pr
- [更新 unmatched 安装文档](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/40)
- [添加 unmatched 失败用例](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/44)
- [添加 HA、kconfigDepDetector 文档](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/57)

issue
- [23.03 unmatched 打开 firefox 卡死](https://gitee.com/openeuler/RISC-V/issues/I6TH5E)
- [23.03 unmatched gdm 报错](https://gitee.com/openeuler/RISC-V/issues/I6TH5H)
- [23.03 unmatched 使用 lightdm 启动 GNOME 报错](https://gitee.com/openeuler/RISC-V/issues/I6TH5M)
- [23.03 mugen 自动化测试报告](https://gitee.com/openeuler/RISC-V/issues/I6U5PF)

在 ros 小组的相关工作:
- [blog 使用 Docker 安装 ROS](https://blog.sakuya.love/archives/ros-install-docker/)
- [blog ROS 学习笔记](https://blog.sakuya.love/archives/ros-learn/)

  正在学习中的打包工作
  - https://gitee.com/SiHuaNRi/pcl_msgs-spec
  - https://gitee.com/SiHuaNRi/rcl-spec

### zhangtianyu

4月份主要工作内容:

- 完成测试小队交接 转入 ROS 小队
- 学习 ros 相关的知识
- 出差到 odd 自我介绍
- 学习打包相关内容

#### 测试小队相关内容

PR

- A-Tune 和 虚拟化 Virt 测试结果：https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/23
- 更新 D1 nezha 安装 openEuler RISC-V 23.03 V1 相关文档：https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/42
- 添加 D1 nezha openEuler RISC-V 23.03 V1 相关失败用例：https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/47
- 更新 DDE, Cinnamon 相关文档，以及工作状态：https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/56

issues

- 23.03 Allwinner D1 Nezha 启动 firefox 失败：https://gitee.com/openeuler/RISC-V/issues/I6TMB6
- 23.03 etmem 无法使用：https://gitee.com/openeuler/RISC-V/issues/I6V7C0

#### ROS 小队：尝试中/进行中/暂未提交
- 学习相关内容：https://gitee.com/zhtianyu/ros-learning
- 尝试打包，分析依赖等：https://build.openeuler.openatom.cn/package/show/home:Tianyu:branches:openEuler:ROS:humble/ros2cli


## Debian

### yubo

4月份主要做了以下几方面的事情:
- 出差到现场支援项目
- port Debian rv32 (感谢gaohan老师)
- 修复了几个riscv64的包
- 配合wangliu老师对Debian riscv64进行机器维护

#### link
- https://github.com/yuzibo/debian_dev/tree/main/firefox/111-0-1 [firefox built on Unmatched]
- https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1012218#30 [firefox 112]
- https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1030895#20 [qt6-webengine update]
- https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1034484 [luajit support rv64]
- https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1030916#30 [qtwebengine-opensource-src update]
- [support riscv32 in check_arch from rebootstrap](https://salsa.debian.org/helmutg/rebootstrap/-/commit/31aa37e39c6b84afe2fc3edfa55e112042372c5b)
- [support riscv32 for linux from rebootstrap](https://salsa.debian.org/helmutg/rebootstrap/-/commit/3c8ccb935a12f59395a1bddc41a7a04a78b44606)

### sunmin
- https://github.com/kdlucas/byte-unixbench/pull/89[byte-unixbench rv64支持] <br />
  https://github.com/kdlucas/byte-unixbench/commit/a07fcc03264915c624f0e4818993c5b4df3fa703
- https://zhuanlan.zhihu.com/p/619410400 [文章jettyweb服务搭建]
- https://lists.debian.org/debian-java/2023/04/msg00000.html [ftbfs bug]
- https://github.com/intel/lmbench/issues/12 [lmbench for riscv] <br />
  https://github.com/sunmin89/lmbench4riscv
### xuyifan

### zhanghaikan

- rustc 1.66.0 merged https://salsa.debian.org/rust-team/rust/-/merge_requests/24
  - 1.67 & 1.68 prepared

### chenxuan

- https://github.com/Sakura286/libreoffice-riscv64-performance-testing/tree/main/benchmark [LibreOffice initial benchmark]

### licheng

- https://mentors.debian.net/package/gl4es/ [RFS: gl4es opengl to opengles translation library]
- https://github.com/DynamoRIO/dynamorio/pull/6014 [Dynamorio implement some base instrutions]

### interns

#### Eric long

#### Liu Yong
- https://salsa.debian.org/rust-team/debcargo-conf/-/merge_requests/488
- https://salsa.debian.org/rust-team/debcargo-conf/-/merge_requests/489
- https://salsa.debian.org/rust-team/debcargo-conf/-/merge_requests/490
- https://salsa.debian.org/rust-team/debcargo-conf/-/merge_requests/491
- 还有几个包因为上游依赖问题未能成功构建，正在处理后提 PR 给上游合入后再提交 Debian
- update rust from 1.63 to 1.82(因跨多个版本问题，仍在处理)
- https://www.bilibili.com/video/BV1Q5411w7z5


## Ubuntu

## Deepin

### gaohan

deepin 社区事务 https://www.freelists.org/list/deepin-devel :
- 2023/04例会部分提案: [社区技术委员会 会议纪要 23/04/26](https://www.freelists.org/post/deepin-devel/-230426)

杂项:
- 协助/审阅deepin-riscv方向pr/obs提交
- 解决obs上deepin-riscv相关依赖问题
- deepin-stage1-beta 桌面同步项目
- 现场项目出差

### interns

#### 桂香伟

##### obs提交
- util-dfm	https://build.tarsier-infra.com/request/show/957
- udisks2-qt5	https://build.tarsier-infra.com/request/show/956
- qt5integration	https://build.tarsier-infra.com/request/show/955
- go-gir-generator	https://build.tarsier-infra.com/request/show/954
- gio-qt	https://build.tarsier-infra.com/request/show/953
- fcitx-configtool	https://build.tarsier-infra.com/request/show/952
- dwayland	https://build.tarsier-infra.com/request/show/951
- dtkwidget	https://build.tarsier-infra.com/request/show/950
- dtkgui	https://build.tarsier-infra.com/request/show/949
- dtkdeclarative	https://build.tarsier-infra.com/request/show/948
- dtkcore	https://build.tarsier-infra.com/request/show/947
- deepin-upgrade-manager	https://build.tarsier-infra.com/request/show/946
- deepin-terminal	https://build.tarsier-infra.com/request/show/945
- deepin-system-monitor	https://build.tarsier-infra.com/request/show/944
- deepin-log-viewer	https://build.tarsier-infra.com/request/show/943
- deepin-kwin	https://build.tarsier-infra.com/request/show/942
- deepin-icon-theme	https://build.tarsier-infra.com/request/show/941
- deepin-home	https://build.tarsier-infra.com/request/show/940
- deepin-fcitx5configtool-plugin	https://build.tarsier-infra.com/request/show/939
- deepin-face	https://build.tarsier-infra.com/request/show/938
- deepin-desktop-theme	https://build.tarsier-infra.com/request/show/937
- deepin-desktop-environment	https://build.tarsier-infra.com/request/show/936
- deepin-deb-installer	https://build.tarsier-infra.com/request/show/935
- dde-widgets	https://build.tarsier-infra.com/request/show/934
- dde-session-ui	https://build.tarsier-infra.com/request/show/933
- dde-session-shell	https://build.tarsier-infra.com/request/show/932
- dde-session	https://build.tarsier-infra.com/request/show/931
- dde-launcher	https://build.tarsier-infra.com/request/show/930
- dde-file-manager	https://build.tarsier-infra.com/request/show/929
- dde-daemon	https://build.tarsier-infra.com/request/show/928
- dde-control-center	https://build.tarsier-infra.com/request/show/927
- dde-clipboard	https://build.tarsier-infra.com/request/show/926
- dde-application-manager	https://build.tarsier-infra.com/request/show/925
- dde-appearance	https://build.tarsier-infra.com/request/show/924
- dde-app-services	https://build.tarsier-infra.com/request/show/923
- dde-account-faces	https://build.tarsier-infra.com/request/show/922
- startdde	https://build.tarsier-infra.com/request/show/901
- dde-network-core	https://build.tarsier-infra.com/request/show/900
- deepin-ocr-plugin-manager	https://build.tarsier-infra.com/request/show/886
- deepin-ocr	https://build.tarsier-infra.com/request/show/885
- dtkmultimedia	https://build.tarsier-infra.com/request/show/884
- ncnn	https://build.tarsier-infra.com/request/show/883
- opencv-mobile	https://build.tarsier-infra.com/request/show/882
- golang-golang-x-sys	https://build.tarsier-infra.com/request/show/914
- xorg-server ​https://build.tarsier-infra.com/request/show/959
- libdrm https://build.tarsier-infra.com/request/show/960

##### github提交
- xorg-server https://github.com/deepin-community/xorg-server/pull/4

#### 凌莞

##### obs提交
- deepin-proxy https://build.tarsier-infra.com/request/show/887
- deepin-desktop-environment https://build.tarsier-infra.com/request/show/888
- deepin-upgrade-manager https://build.tarsier-infra.com/request/show/889
- deepin-desktop-theme https://build.tarsier-infra.com/request/show/891
- deepin-desktop-base https://build.tarsier-infra.com/request/show/892
- dtkcore https://build.tarsier-infra.com/request/show/893
- startdde https://build.tarsier-infra.com/request/show/894
- dde-daemon https://build.tarsier-infra.com/request/show/895
- dde-session https://build.tarsier-infra.com/request/show/896
- dtkgui https://build.tarsier-infra.com/request/show/897
- dwayland https://build.tarsier-infra.com/request/show/898
- dtkdeclarative https://build.tarsier-infra.com/request/show/899
- dde-app-services https://build.tarsier-infra.com/request/show/902
- dde-appearance https://build.tarsier-infra.com/request/show/903
- dde-application-manager https://build.tarsier-infra.com/request/show/904
- dde-clipboard https://build.tarsier-infra.com/request/show/905
- dde-control-center https://build.tarsier-infra.com/request/show/906
- dde-launcher https://build.tarsier-infra.com/request/show/907
- dde-session-shell https://build.tarsier-infra.com/request/show/908
- dde-session-ui https://build.tarsier-infra.com/request/show/909
- dde-widgets https://build.tarsier-infra.com/request/show/910
- deepin-deb-installer  https://build.tarsier-infra.com/request/show/911
- deepin-face https://build.tarsier-infra.com/request/show/912
- deepin-fcitx5configtool-plugin https://build.tarsier-infra.com/request/show/913
- deepin-home https://build.tarsier-infra.com/request/show/914
- deepin-kwin https://build.tarsier-infra.com/request/show/915
- deepin-log-viewer https://build.tarsier-infra.com/request/show/916
- deepin-system-monitor https://build.tarsier-infra.com/request/show/917
- deepin-terminal https://build.tarsier-infra.com/request/show/918
- downloadmanager https://build.tarsier-infra.com/request/show/919
- dtkwidget https://build.tarsier-infra.com/request/show/920
- qt5integration https://build.tarsier-infra.com/request/show/921

##### GitHub
- https://github.com/linuxdeepin/deepin-desktop-base/pull/56

## OpenAnolis

## Fedora

### 1、 Fedora 38 的编包已经接近80%
所有的关键软件版本都追随Fedora 38最新的版本，继续编包中。支持多款桌面系统：
 * XFCE
 * LXDE
 * LXQT[need test]
 * budgie[need test]
 * cinnamon[need test]
 * sugar[need test]
 * mate[need test]
 * phosh[need test]
### 2、support spins:
 * server ： [Fedora 38 Server Image for Sophgo SG2042](http://openkoji.iscas.ac.cn/kojifiles/work/tasks/7288/1287288/fedora-disk-server_sophgo_sg2042-f38-20230510-030133.n.0-sda.raw.xz)
 * cloud ：
 * Workstation： [Fedora 38 XFCE/LXDE/LXQT desktop Image for Sophgo SG2042](http://openkoji.iscas.ac.cn/kojifiles/work/tasks/4611/1284611/fedora-disk-developer-xfce_sophgo_sg2042-f38-20230510-030031.n.0-sda.raw.xz)
 * Iot [Ongoing]
 * CoreOS [Ongoing]
### 3 、ROS/ROS2的软件编译和安装在Fedora 38 进行中。

### 张文宇：
- upstream Fedora srpm patch：
Boost： https://src.fedoraproject.org/rpms/boost/pull-request/16
CMake： https://src.fedoraproject.org/rpms/cmake/pull-request/26
- 参与F38打包，参与解决golang模块包问题

### 刘海滨：
- 1、对ROS1依赖sbcl进行移植编译和测试，一起查找问题原因，并已经定位了问题，并写[总结文档](https://github.com/fedora-riscv/obs/blob/main/README.md)。
- 2、ROS1编译：完成了从编译和文档的过程，正在测试。[经验文档](https://github.com/fedora-riscv/sbcl-build-docs/blob/main/patch.MD)
3,对Fedora安装image进行研究和调试,[过程文档](https://gitee.com/fedora-rpms/installer-docs/blob/master/README.md)

### 赵佳盛：
- 1、跟踪ROCKS的编译进度[文档](https://github.com/fedora-riscv/rocks_track_doc)。
- 2、学习并翻译 [Fedora Release Engineering 文档](https://github.com/fedora-riscv/releng.git)，已完成文档如下:
 * sop_adding_packages_compose_artifact
 * sop_adding_side_build_targets
 * sop_bodhi_activation
 * sop_branch_freeze
 * sop_branching
 * sop_breaking_development_freeze
 * sop_clean_amis
 * sop_composing_fedora
 * sop_create_release_signing_key
 * sop_deprecate_ftbfs_packages
 * sop_end_of_life
 * sop_eol_change
 * sop_mass_branching
 * sop_mass_rebuild_modules
 * sop_mass_rebuild_packages
- 3、管理  Openkoji 编译 tags, 接手管理[Fedora 38 tags 组](https://openkoji.iscas.ac.cn/koji/tags)

## FreeBSD

## OpenSUSE

## OpenKylin

## OpenCloudOS

## RT-Thread
### liyangyang

### yanghaiyong

### liuyuan

### chushicheng

**pr**:

- https://github.com/RT-Thread/rt-thread/pull/7147
- https://github.com/RT-Thread/rt-thread/pull/7385
- https://github.com/RT-Thread/rt-thread/pull/7397
- https://github.com/RT-Thread/rt-thread/pull/7406
- https://github.com/RT-Thread/rt-thread/pull/7408

## 第三测试小队

### 1.规划和内部培训

#### 1.1 openEuler 23.03 测试报告（Tarsier每周技术分享）

#### 1.2 [openEuler RISC-V 测试策略分析（ODD会议现场调研结果，Tarsier内部报告）](https://github.com/ArielHeleneto/Work-PLCT/blob/master/ODD2023%E8%B0%83%E7%A0%94%E7%BB%93%E6%9E%9C.md)

### 2.系统测试

#### 2.1 openEuler 2303 测试（继上个月测试，以下内容修改和新加）

- [总报告更新](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/blob/master/README.md)

- [openEuler RISC-V 基础测试，性能测试，根据上游的测试反馈](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/BasicTest)
  - fio

  - libMicro

  - lmbench

  - stream

  - unixbench 

- [Mugen自动化测试](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Automated_Test/Mugen)

  本次测试共399个（相相比22.03 preview V2测试增加了70个）测试套(396个软件包+systemd+os-basic+os-storage)，2127个测试用例（相比22.03 preview V2测试增加了701个）

- [A-Tune](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/A-Tune)

- [Chromium](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Chromium)

- [Cinnamon](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Cinnamon)

- [iSulad等虚拟化测试](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Containers)

- [DDE](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/DDE)

- [Firefox](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Firefox)

- [GCC](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/GCC)

- [GIMP](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/GIMP)

- [Gnome](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/GIMP)

- [HA](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/HA)

- [JDK](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/JDK)

- [Kiran](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Kiran)

- [LibreOffice](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/LibreOffice)

- [MYSQL](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/MYSQL)

- [NestOS](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/NestOS)

- [SYSTEM](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/SYSTEM)

- [Sysprof](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Sysprof)

- [System_Dependency](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/System_Dependency)

- [UKUI](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/UKUI)

- [Virt](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Virt)

- [XFCE](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/XFCE)

- [astream](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/astream)

- [k3s](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/k3s)

- [kconfigDetector](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/kconfigDetector)

- [kubernetes](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/kubernetes)

- [libcareplus](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/libcareplus)

- [mindspore](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/mindspore)

- [oncn-bwm](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/oncn-bwm)

- [openResty](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/openResty)

- [pkgship](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/pkgship)

- [secGear](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/secGear)

- [secPaver](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/secPaver)

- [国密算法](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/%E5%9B%BD%E5%AF%86%E7%AE%97%E6%B3%95)

- [硬件（开发板）兼容性测试，添加测试用例](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Hardware_Test)

#### 2.4 安装和使用文档（修改或新加）

- StarFive VisionFive 2 安装文档更新，补充安装系统至 NVMe 硬盘方法 [docs](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Installation_Book/Visionfive2)
- UKUI 使用文档 [docs](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/User_Book/UKUI使用手册)
- Kiran 使用文档 [docs](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/User_Book/Kiran使用手册)
- JDK 安装文档 [docs](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/JDK)

#### 2.3 缺陷

- [缺陷issue入口](https://gitee.com/openeuler/RISC-V/issues)
  - 22.03 Firefox 设置中的‘搜索’缺陷
  - 23.03 6.1 内核下无法启动 Firefox
  - DDE 桌面环境登陆后崩溃
  - 23.03 etmem 无法使用
  - 23.03 mugen 自动化测试缺陷报告
  - 23.03kubekey部署 KubeSphere 集群失败
  - 23.03 Allwinner D1 Nezha 启动 firefox 失败：Illegal instruction
  - 23.03 licheePi rv dock 启动 chromium 无法显示
  - 23.03 unmatched 使用 lightdm 启动 GNOME 报错
  - 23.03 unmatched gdm 报错
  - 23.03 unmatched 打开 firefox 卡死
  - 23.03 qemu ukui 桌面极为卡顿，无法启动任意gui 程序
  - 23.03 vision five安装ukui桌面，进入桌面，两个画面循环显示
  - licheePI rv dock 无法启动 libreoffice
  - 23.3 licheePI rv dock 无法启动 firefox: Illegal instruction

### 3. 自动化测试工具

#### 3.1 openQA开发

- openQA xfce 测试 [测试](https://gitee.com/lvxiaoqian/os-autoinst-distri-openeuler/pulls/4) [Needles](https://gitee.com/lvxiaoqian/os-autoinst-needles-openeuler/pulls/3) [文档更新](https://github.com/ArielHeleneto/RISCV-testcase/pull/59)
- GIMP Perl 用例编写：见 [openqa 服务器](https://openqa.ariels.xyz/)(完成了部分（6/30））
- GIMP Perl 用例修改：https://github.com/GICEGreenIce/PLCTstuff/tree/main/gimp
- GIMP 用例提交：https://gitee.com/lvxiaoqian/os-autoinst-distri-openeuler/pulls/5
- GIMP 用例 needle 提交：https://gitee.com/lvxiaoqian/os-autoinst-needles-openeuler/pulls/4

#### 4.2 Mugen在openEuler下的开发

- openEuler RISC-V 23.03 mugen embedded和smoke[测试](https://github.com/brsf11/Tarsier-Internship/commit/e10dd6f0a92c966b227107d1afcd6e5483015023)，新增了多机器和网卡的支持
- result_parser脚本[新增](https://github.com/brsf11/mugen-riscv/commit/3292037f32d28a645a87601aaa428c051e93e349)文件名替换功能，替换log文件的：，以适配windows

- 统计了过往测试结果，[新增](https://github.com/brsf11/mugen-riscv/commit/26429894951d2e913f429970d1a7a6a8ab056591)mugen测试用例状态追踪，为以后和QA sig对接做准备

- 进行2303预测试base部分的mugen上游测试（QA sig对接做准备）, [logs](https://github.com/KotorinMinami/plct-working/blob/main/openEuler-test/2303basetest)

- 修改mugen_riscv.py，增加其指定日志输出路径功能, [commit](https://github.com/KotorinMinami/mugen-riscv/commit/50d9a3cf8f62f032224694fbfb1ab13e74a05ef0#diff-b41b734ad16d09679b0f97d75ba1a63d5262e09d86a671c6b80302ddf7b2583d)
- 修改qemu_test.py，拓宽host和guest间文件传输方式，并使用此方式尝试解决scp无法使用的情形, [commit](https://github.com/KotorinMinami/mugen-riscv/commit/50d9a3cf8f62f032224694fbfb1ab13e74a05ef0#diff-56a86f9f01cddc9f2e16d54f11d84ca40ff7ed5f73a281c9e624d27fb750e18e)

### 4. 测试用例库建设

#### 4.1 测试用例 / testcases

  - [容器相关](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Containers)

  - Docker
  - iSulad （包括 Native Network / CNI 网络特性）
  - 系统容器
  - 安全容器
  - 容器镜像构建

#### 4.2 其他软件包

  - libcareplus [testcase](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/libcareplus)
  - kconfigDetector [testcase](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/kconfigDetector)

#### 4.3 硬件测试

- StarFive VisionFive 2 [testcase](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Hardware_Test/Visionfive2)

#### 4.4 netperf

- netperf 测试：4月29日给到测试结果(https://github.com/GICEGreenIce/PLCTstuff/tree/main/netperf)

### 5. 独立测试或（修复）任务

- Firefox 自动化测试框架调研和测试 [click here](https://github.com/microseyuyu/PLCT-Work-repository/blob/main/Firefox/build_and_test.log)
- VLC 自动化测试框架调研和测试 [click here](https://github.com/microseyuyu/PLCT-Work-repository/blob/main/vlc/build_and_test.log)

- 增加密钥长度选项以解决 libkcapi 在使用 SM4 算法进行加密时由于密钥长度错误造成 的 EINVAL [Issue #150](https://github.com/smuellerDD/libkcapi/issues/150) [Pull Request](https://github.com/smuellerDD/libkcapi/pull/151)
- openEuler libkcapi 包应用补丁 [Pull Request](https://gitee.com/src-openeuler/libkcapi/pulls/49)


### 组员

#### 郑景坤 @KevinMX

##### 缺陷报告

- [I6R1UT](https://gitee.com/openeuler/RISC-V/issues/I6R1UT) iSulad 无法启动 - Fixed
- [I6R1UT](https://gitee.com/openeuler/RISC-V/issues/I6RDWA) isula-build 服务无法启动 - Fixed
- [I6RSAC](https://gitee.com/openeuler/RISC-V/issues/I6RSAC) lxcfs 服务无法启动 - Fixed
- [I6R9OQ](https://gitee.com/openeuler/RISC-V/issues/I6R9OQ) kata-containers 软件包缺失 - Confirmed, WIP
- [I6WHYU](https://gitee.com/openeuler/RISC-V/issues/I6WHYU) 23.03 Kiran 桌面安装并重启后默认不会开启网络连接 - Confirmed, WIP
- [I6WRSY](https://gitee.com/openeuler/RISC-V/issues/I6WRSY) 23.03 6.1 内核下无法启动 Firefox - TBD

##### 测试用例

- 容器 (Docker, iSulad, etc): [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/Containers)
- libcareplus: [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/libcareplus)
- kconfigDetector: [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/kconfigDetector)
- StarFive VisionFive 2 硬件测试：[Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Hardware_Test/Visionfive2)

##### 文档内容

- StarFive VisionFive 2 安装文档更新，补充安装系统至 NVMe 硬盘方法 [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/Installation_Book/Visionfive2)
- UKUI 使用文档 [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/User_Book/UKUI%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C)
- Kiran 使用文档 [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/User_Book/Kiran%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C)
- JDK 安装文档 [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/tree/master/System_and_Feature_Test/JDK)

##### 合并请求 / Pull Requests

补全部分安装文档/使用文档/测试用例：

[!36 认领&更新：容器/containers](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/36)

[!51 更新 VisionFive 2 测试用例及安装说明](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/51)

[!61 文档更新](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/61)

[!62 更新 UKUI 文档，认领任务](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/62)

[!64 更新部分使用文档和测试用例](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/pulls/64)

src-openEuler/lmbench:

[!18 Update gnu-os and spec](https://gitee.com/src-openeuler/lmbench/pulls/18) 


##### 其他内容

- 重新整理 openEuler RISC-V 23.03 的测试内容，对于不清楚的部分需求进行调研，对先前未完成的部分软件包在 23.03 进行回归测试，整理所有软件包对应 SIG 组，并对已测试软件包可用性进行统计。

  - 结果见 [此处](https://github.com/KevinMX/PLCT-Tarsier-Works/blob/main/misc/month1/what-is-this.md)。

- 针对 QA 组测试需求进行基础测试，目前已完成 unixbench、libmicro、fio、stream、lmbench。

> TODO: 部分测试方法/软件包针对 RISC-V 平台情况有所修改，需要确认 sig-QA 能否接受。

  - 结果见 [Gitee](https://gitee.com/yunxiangluo/openeuler-riscv-2303-test/blob/master/BasicTest/res.md)。

## PTS Support

## Perf

## Arch Linux & Gentoo & NixOS

Tracked by PLCT Lab

### jinqiang zhang (Gentoo)

#### 常用软件包以及firefox/ceph等的升级、编译观测:
1. [Bug 904532](https://bugs.gentoo.org/904532) www-client/firefox-112.0.1: riscv: File listed in UNIFIED_SOURCES does not exist: js/src/jit/riscv64/AssemblerMatInt.cpp.cpp
    - [www-client/firefox: add a riscv64 patch for 112.0.1](https://github.com/gentoo/gentoo/commit/62a2d3069936)
#### keyword/rekeyword bug:
1. [Bug 904415](https://bugs.gentoo.org/904415) dev-libs/kirigami-addons, dev-libs/ktextaddons, dev-libs/qcoro5, app-editors/ghostwriter, net-im/tokodon, net-im/neochat keywordreq
    - [dev-libs/ktextaddons: Keyword 1.2.0 riscv, #904415](https://github.com/gentoo/gentoo/commit/642fc8a1eaae)
    - [dev-libs/qcoro5: Keyword 0.8.0 riscv, #904415](https://github.com/gentoo/gentoo/commit/97400fac172b)
    - [dev-libs/kirigami-addons: Keyword 0.7.2 riscv, #904415](https://github.com/gentoo/gentoo/commit/5e1bf8aeffd7)
2. [Bug 904147](https://bugs.gentoo.org/904147) dev-ruby/shoulda:3: riscv keywording
    - [dev-ruby/shoulda: Keyword 3.6.0-r2 riscv, #904147](https://github.com/gentoo/gentoo/commit/6ac580ecd804)
    - [dev-ruby/shoulda-context: Keyword 1.2.2-r1 riscv, #904147](https://github.com/gentoo/gentoo/commit/b3674c0dea65)
3. [Bug 761550](https://bugs.gentoo.org/761550) dev-ruby/http:5 and new dependencies keywording request
    - [dev-ruby/rails: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/9cc026cf66af)
    - [dev-ruby/actionmailbox: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/f562777c56b3)
    - [dev-ruby/actionmailer: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/cc626d1ac604)
    - [dev-ruby/actiontext: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/ef65b612c4da)
    - [dev-ruby/actioncable: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/442d7ce288dc)
    - [dev-ruby/propshaft: Keyword 0.7.0-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/e3a30c2fbec6)
    - [dev-ruby/sinatra: Keyword 3.0.5-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/e0d4f2eb92e9)
    - [dev-ruby/actionview: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/7a01459e1c49)
    - [dev-ruby/haml: Keyword 6.1.1-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/7e6c51cfc9b3)
    - [dev-ruby/activestorage: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/ca92fb7c7445)
    - [dev-ruby/sprockets-rails: Keyword 3.4.2-r2 riscv, #761550](https://github.com/gentoo/gentoo/commit/9b8fc50480cd)
    - [dev-ruby/activerecord: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/47eda5841949)
    - [dev-ruby/actionpack: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/c06064cb380d)
    - [dev-ruby/activejob: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/e6203bedd147)
    - [dev-ruby/globalid: Keyword 1.1.0-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/b72beaf1646d)
    - [dev-ruby/railties: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/2dd5188d8503)
    - [dev-ruby/activemodel: Keyword 7.0.4.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/5e30f3d3cfea)
    - [dev-ruby/async-container: Keyword 0.16.12-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/3bfb0be37dfa)
    - [www-servers/puma: Keyword 6.1.1-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/56c39169094d)
    - [dev-ruby/localhost: Keyword 1.1.10 riscv, #761550](https://github.com/gentoo/gentoo/commit/ba5ad58e29b8)
    - [dev-ruby/sus-fixtures-async: Keyword 0.1.1 riscv, #761550](https://github.com/gentoo/gentoo/commit/94648315f9d3)
    - [dev-ruby/async-process: Keyword 1.3.1 riscv, #761550](https://github.com/gentoo/gentoo/commit/7df22a98a0d7)
    - [dev-ruby/async-rspec: Keyword 1.16.1-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/de3467dd0d12)
    - [dev-ruby/rspec-memory: Keyword 1.0.3 riscv, #761550](https://github.com/gentoo/gentoo/commit/66750c877893)
    - [dev-ruby/rspec-files: Keyword 1.1.3 riscv, #761550](https://github.com/gentoo/gentoo/commit/1e0c678a3674)
    - [dev-ruby/async-io: Keyword 1.34.3-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/4d43c1674288)
    - [dev-ruby/async: Keyword 2.5.0 riscv, #761550](https://github.com/gentoo/gentoo/commit/0bb21729d504)
    - [dev-ruby/async: Keyword 1.31.0 riscv, #761550](https://github.com/gentoo/gentoo/commit/d5bfeaacbcf4)
    - [dev-ruby/io-event: Keyword 1.1.7 riscv, #761550](https://github.com/gentoo/gentoo/commit/f332bf571527)
    - [dev-ruby/timers: Keyword 4.3.5 riscv, #761550](https://github.com/gentoo/gentoo/commit/057074b483e6)
    - [dev-ruby/console: Keyword 1.16.2-r1 riscv, #761550](https://github.com/gentoo/gentoo/commit/c8323fba2443)
    - [dev-ruby/fiber-local: Keyword 1.0.0 riscv, #761550](https://github.com/gentoo/gentoo/commit/ad88268d0c88)

    修复相关测试用例:
    - [dev-ruby/activejob: slot 7.0 add dev-ruby/zeitwerk test depend](https://github.com/gentoo/gentoo/commit/911db0e83493)
    - [dev-ruby/async-io: fix 1.34.3 tests](https://github.com/gentoo/gentoo/commit/4e187139bebf)
    - [dev-ruby/actionmailbox: add actioncable test depend](https://github.com/gentoo/gentoo/pull/30555)
4. [Bug 903715](https://bugs.gentoo.org/903715) dev-pyton/hatchling: rekeyword
    - [dev-python/hatchling: Keyword 1.14.0 riscv, #903715](https://github.com/gentoo/gentoo/commit/77f58c10d277c49ffb660a5f79f7774b4d001b51)
    - [dev-python/trove-classifiers: Keyword 2023.3.9 riscv, #903715](https://github.com/gentoo/gentoo/commit/240040efda4635f3874a0f949e7ebadf38474df0)
    - [dev-python/calver: Keyword 2022.06.26 riscv, #903715](https://github.com/gentoo/gentoo/commit/3fb5a646447c65a2f3cb2cc8d1bd538c01a01490)
5. [Bug 903714](https://bugs.gentoo.org/903714) dev-ruby/instance_storage: rekeywording
    - [dev-ruby/instance_storage: Keyword 2.0.0 riscv, #903714](https://github.com/gentoo/gentoo/commit/d1117f9f4ff885b7f847443472644143ae113396)
6. [Bug 903741](https://bugs.gentoo.org/903741) app-portage/elsw: ~arm, ~arm64 and ~riscv keywording request
    - [app-portage/elsw: Keyword 0.0.0-r1 riscv, #903741](https://github.com/gentoo/gentoo/commit/946a17e1c119af8893639800989c8f7361209f34)

## AOSP

- Google AOSP upstream 工作更新:
  - AOSP upsteam RISC-V 进展进度跟踪，每两周更新一次。
    - Android (RISC-V) Review 双周报（第 14 期）：<https://zhuanlan.zhihu.com/p/621530337>
    - Android (RISC-V) Review 双周报（第 15 期）：<https://zhuanlan.zhihu.com/p/625260114>

- RVI Android SIG 工作更新：
  - Chromium for Android apk 109.0.5414.87 移植
    
    本月完成了 ContentShell apk 的 build，可以正常加载网页和播放视频。
    下个月计划：
    - 继续 ContentShell 的单元测试
    - 解决 Chromium 的 crash 问题

    相关 PR list 如下：
    - [[chrome.angle] Force Pure RISCV64 Support on Android](https://github.com/aosp-riscv/angle/pull/5)
    - [[chrome] Force Pure RISCV64 Support on Android](https://github.com/aosp-riscv/chromium/pull/30)
    - [[chrome] rollback to use API level 24](https://github.com/aosp-riscv/chromium/pull/34)
    - [[chrome] crashpad: riscv -> riscv32](https://github.com/aosp-riscv/chromium/pull/35)
    - [[chrome] fixed crashpad crash issue](https://github.com/aosp-riscv/chromium/pull/36)
    - [[chrome] continue fixed crashpad crash issue](https://github.com/aosp-riscv/chromium/pull/37)
    - [[chrome.ndk] revert change to keep align with ndk 22/23](https://github.com/aosp-riscv/toolchain-prebuilts-ndk-r23/pull/1)
    - [[chrome] added target content_shell_apk in Makefile](https://github.com/aosp-riscv/chromium/pull/38)
    - [[chrome] updated doc about env setup](https://github.com/aosp-riscv/working-group/pull/54)
    - [[chrome] fixed document issue](https://github.com/aosp-riscv/working-group/pull/56)
    - [[chrome.clang] merge two ndk into one](https://github.com/aosp-riscv/chromium/pull/41)
    - [[chrome] merge riscv64_109.0.5414.87_dev_clang into riscv64_109.0.5414.87_dev](https://github.com/aosp-riscv/chromium/pull/42)
    - [[chrome.doc] use only one ndk for chrome build](https://github.com/aosp-riscv/working-group/pull/57)
    - [[chrome] enable proprietary codecs](https://github.com/aosp-riscv/chromium/pull/44)

- 技术类文章分享：
  - musl 构建说明:<https://zhuanlan.zhihu.com/p/618927133>
  - musl 交叉调试说明:<https://zhuanlan.zhihu.com/p/619028851>
  - musl 的 build system 分析:<https://zhuanlan.zhihu.com/p/619260551>
  - 静态链接可执行程序的入口分析:<https://zhuanlan.zhihu.com/p/619674333>
  - 介绍一个方便 riscv for aosp 测试的开发环境:<https://zhuanlan.zhihu.com/p/620131525>
  - RISC-V 中的 global pointer 寄存器:<https://zhuanlan.zhihu.com/p/620412728>
  - 学习笔记：Position-Independent Executables:<https://zhuanlan.zhihu.com/p/620935366>
  - 学习笔记：Shadow Call Stack:<https://zhuanlan.zhihu.com/p/623061815>
  - Global Constructors/Destructors:<https://zhuanlan.zhihu.com/p/623860730>
  - 学习总结：C Runtime:<https://zhuanlan.zhihu.com/p/624560166>

## 资源和链接

- 待添加
